...
@export var wheel_target: Marker3D
...
func _process(delta: float) -> void:
	if dragging and rigidbody is Wheel:
		...
		if pos is Vector3:
			var vec = pos - wheel_target.global_position
			var vec_len = vec.length()
			vec = (vec / vec_len) * drag_strength
			rigidbody.apply_force(vec * delta, wheel_target.global_position - rigidbody.global_position)
